...
// import { TodosSelectors } from '../Redux/TodosRedux'

export function * getTodos (api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(TodosSelectors.getData)
...
export function * addTodo (api, action) {
  ...
  // const currentData = yield select(TodosSelectors.getData)
  // make the call to the api
  const response = yield call(api.addtodo, data)

  // success?
  if (response.ok) {
    // You might need to change the response here - do this with a 'transform',
    // located in ../Transforms/. Otherwise, just pass the data back from the api.
    yield put(TodosActions.todosAddSuccess(response.data))
  } else {
    yield put(TodosActions.todosAddFailure())
  }
}
